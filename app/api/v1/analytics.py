...
FIX v2:
  - Auth: dependencia opcional — acepta JWT interno O sin token (dev/hackathon).
    En producción sustituye _dep_gerente_opcional por requerir_gerente.
"""
...
# ---------------------------------------------------------------------------
# Auth flexible (JWT interno ó sin token en dev)
# ---------------------------------------------------------------------------

_esquema_opcional = HTTPBearer(auto_error=False)


async def _dep_auth_opcional(
    credenciales: Optional[HTTPAuthorizationCredentials] = Depends(_esquema_opcional),
    db=Depends(obtener_db),
) -> Optional[Usuario]:
    """Acepta Bearer <JWT> o ningún header. Nunca bloquea la request."""
    if not credenciales:
        return None
    payload = decodificar_token(credenciales.credentials)
    if not payload or payload.get("type") != "access":
        return None
    return await db.obtener(COLECCIONES["users"], payload.get("sub"), Usuario)
...
def _metricas_ordenes(ordenes: list) -> Dict[str, float]:
    ...
    return {"avg_order_value": round(avg_valor, 4),
            "kitchen_queue_depth": round(min(cola / 20.0, 1.0), 4),
            "avg_prep_time": 0.40, "order_error_rate": 0.05}
...
def _metricas_reservaciones(reservaciones: list) -> Dict[str, float]:
    ...
    return {"cancellation_rate": round(min(canceladas / total, 1.0), 4),
            "return_rate": 0.45, "complaint_rate": 0.03, "avg_wait_time": 0.30}
...
async def _construir_contexto(db, restaurant_id: str) -> Dict[str, Any]:
    ...
    reservaciones = await db.listar(COLECCIONES["reservations"], Reservacion,
                                    [("restaurant_id", "==", restaurant_id)])
    activas   = [r for r in reservaciones if getattr(r, "status", None) in ("confirmed", "pending")]
    ordenes   = await db.listar(COLECCIONES["orders"], Orden,
                                [("status", "in", [EstadoOrden.pendiente.value,
                                                   EstadoOrden.preparando.value])])
    mo = _metricas_ordenes(ordenes)
    mr = _metricas_reservaciones(reservaciones)
    tod = (ahora.hour * 60 + ahora.minute) / 1440.0

    return {
        "time_of_day": round(tod, 4),         "day_of_week": round(ahora.weekday() / 6.0, 4),
        "is_holiday": 0.0,                    "weather_score": 0.6,
        "local_events": min(len(activas) / 30.0, 1.0),
        "avg_order_value": mo["avg_order_value"], "top_category_ratio": 0.40,
        "alcohol_ratio": 0.20,                "dessert_ratio": 0.15, "combo_rate": 0.25,
        "avg_prep_time": mo["avg_prep_time"],  "staff_load": tasa,
        ...
        "avg_rating": 0.80,
        "cancellation_rate": mr["cancellation_rate"], "return_rate": mr["return_rate"],
        "complaint_rate": mr["complaint_rate"],       "avg_wait_time": mr["avg_wait_time"],
        "occupancy_ratio": round(tasa, 4),
        "avg_table_utilization": round(tasa * 0.9, 4),
        "dead_zone_ratio": 0.10, "aisle_blockage": 0.05, "floor_score": 0.75,
    }
...
async def resumen_dashboard(
    ...
    return {
        "restaurant_id": restaurant_id,
        "mesas": {"total": n, "ocupadas": ocupadas, "disponibles": disponibles,
